...
router.get('/', authenticate, async (req: Request, res: Response) => {
    try {
        ...
        const vacationEntries = await Absence.find({
            where: {
                staff: {
...
router.post("/", authenticate, async (req: Request, res: Response) => {
    try {
        ...
        const { startDate, start, endDate, end, techcode} = req.body;
        if (!user) {
        ...
        newEntry.type_techcode = techcode;
        newEntry.response = "Request successfully submitted! Please wait for your Supervisor to reply!";
